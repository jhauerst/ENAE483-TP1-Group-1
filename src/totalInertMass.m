...
function [InertMass]=totalInertMass(stage1,stage2,names,M0,h)
    %% Stage one
    if strcmp(names(3),"Solid")
        ...
        totalMass1 = Interstage+Aft1+propellantTank+insul_solid+engineMass1+structureMass1+gimbalsMass1+wiringMass1;
    else 
        ...
        %find Wiring mass
        [wiringMass1,~] = findWiringa_AvionicsMass(M0,stage1(1));
        totalMass1 = Intertank1+Interstage+Aft1+fuel_tank1+oxidizer_tank1+insul_oxid1+insul_fuel1+engineMass1+structureMass1+gimbalsMass1+wiringMass1;
    end 
    ...
    if strcmp(names(6),"Solid")
        ...
        [Payload2, ~ ,~ , ~, ~] = findFairingMass(stage1(2),stage2(2),h);
        %Propellant mass = solid fuel
        solidM2 = array2(5);
        %DENSITY OF SOLID PROPELLANT SHOULD BE STORED IN FUEL DENSITY
        ...
        totalMass2 = Payload2+propellantTank2+insul_solid2+engineMass2+structureMass2+gimbalsMass2+wiringMass2+avionicsMass2;
    else 
        %Fairing mass
        [payload2, ~, Intertank2, ~, ~] = findFairingMass(stage1(2),stage2(2),h);
        %Fuel mass & Oxidizer masses
        %Pass propellant mass and ratio
        ...
        [~,avionicsMass2] = findWiringa_AvionicsMass(M0,stage2(1));
        [wiringMass2,~] = findWiringa_AvionicsMass(stage2(10),(stage2(1)+13+h));
        totalMass2 = payload2+Intertank2+fuel_tank2+oxidizer_tank2+insul_oxid2+insul_fuel2+engineMass2+structureMass2+gimbalsMass2+wiringMass2+avionicsMass2;
    end
    InertMass = totalMass1 + totalMass2;
end