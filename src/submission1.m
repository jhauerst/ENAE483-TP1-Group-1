function [ Mo_min, Min1_min,Min2_min,Mo1_min, Mo2_min,Mpr1_min,Mpr2_min,chi_min] = submission1 (delta, Propellantstage1, Propellantstage2, useMinMass)
...
for i = 1:length(propNames) %run through all propellant names 
    ...
    [~, mIndex] = findMinMass(Mo);

    %getting all the minimum cost points for the different mass parameters
    Mo_min = Mo(mIndex);
    Min1_min = m_in1(mIndex);
    Min2_min = m_in2(mIndex);
    Mo1_min = M01_array(mIndex);
    Mo2_min = M02_array(mIndex);
    Mpr1_min = m_pr1(mIndex);
    Mpr2_min = m_pr2(mIndex);
    chi_min = chi_array(mIndex);
    end
end